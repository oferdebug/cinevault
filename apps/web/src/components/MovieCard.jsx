...
const IconBookmark = ({ filled }) => (
	<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
	...
	const handleVaultClick = async (event) => {
		...
		try {
			...
			if (err?.message?.includes("vault_limit_reached")) {
				alert("Free plan is limited to 20 titles. Upgrade to Plus for unlimited.");
			} else {
