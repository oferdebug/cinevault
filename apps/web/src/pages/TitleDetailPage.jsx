    ...
    <main className="min-h-screen bg-primary">
      {backdrop && (
        <div className="relative h-[60vh] w-full overflow-hidden">
          <img src={backdrop} alt={name} className="w-full h-full object-cover" />
      ...
      <div className="max-w-6xl mx-auto px-8 -mt-32 relative z-10 pb-16">
        <div className="flex gap-8 flex-col sm:flex-row">
          <img src={poster} alt={name} className="w-48 rounded-2xl shadow-2xl shadow-black/50 shrink-0" />
          ...
          <div className="flex flex-col justify-end">
            ...
            <div className="flex items-center gap-3 text-sm text-gray-100 mb-4 flex-wrap">
              <span className="text-accent font-bold text-base">⭐ {title.vote_average?.toFixed(1)}</span>
              <span>•</span>
              <span>{year}</span>
              <span>•</span>
              <span className="capitalize">{type}</span>
              {title.runtime && <><span>•</span><span>{title.runtime} min</span></>}
            </div>
            <p className="text-light-200 leading-7 max-w-2xl">{title.overview}</p>
            ...
            <div className="grid grid-cols-2 xs:grid-cols-4 md:grid-cols-8 gap-4">
              {cast.map((person) => (
                <div key={person.id} className="text-center">
                  ...
                  />
                  <p className="text-xs text-light-100 font-medium line-clamp-1">{person.name}</p>
                  <p className="text-xs text-gray-100 line-clamp-1">{person.character}</p>
                </div>
              ))}
            </div>
          </section>
